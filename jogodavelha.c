#include <stdio.h>
#include <stdlib.h> //Biblioteca para movimentar as variáveis de memória 
#include <locale.h>

int main(){
setlocale(LC_ALL,""); //para adicionar acentos e as as regras do portugues
    int l, c, linha, coluna, jogador, ganhou, jogadas, opcao;
    char jogo[3][3];

    do{ // para repetir o menu
        jogador = 1;
        ganhou = 0;
        jogadas = 0;
        for(l = 0; l < 3; l++){
        ...
        do{ // Para repetir até alguem ganhar ou atingir o máximo partidas (9) 
            printf("\n\n\t 0   1   2\n\n");
            for(l = 0; l < 3; l++){
                for(c = 0; c < 3; c++){
                    if(c == 0)
